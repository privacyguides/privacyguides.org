...
That said, the [F-droid repository](https://f-droid.org/en/packages/), as well as the [IzzyOnDroid repository](https://apt.izzysoft.de/fdroid/) are home to a lot of apps, so you can use them as a way to search for and discover open-source apps that you can then download through Play Store, Aurora Store, or by getting the APK from the developer directly. It is important to keep in mind that some apps in these repositories have not been updated in years and may rely on unsupported libraries among other things, which poses a potential security risk. You should use your best judgement when looking for new apps via this method.
