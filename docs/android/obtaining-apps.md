...
<p class="admonition-title">F-Droid Basic</p>
 ...
 In some rare cases, an app may only be available through F-Droid. ([Gadgetbridge](../health-and-wellness.md#gadgetbridge)) was historically one example, although it now also publishes developer-signed releases through IzzyOnDroid and its own releases page. If you really need an app like that, we recommend using the newer [F-Droid Basic](https://f-droid.org/en/packages/org.fdroid.basic) client instead of the original F-Droid app to obtain it. F-Droid Basic supports automatic background updates without privileged extension or root, and has a reduced feature set (limiting attack surface).
