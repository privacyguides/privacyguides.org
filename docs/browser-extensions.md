...
uBlock Origin Lite only receives block list updates whenever the extension is updated from your browser's extension marketplace, as opposed to on demand. This means that you may miss out on new threats. The extensions updates every 2 days, using Google [expedited review for filters updates](https://developer.chrome.com/docs/webstore/skip-review). However, only so-called [safe-rules](https://developer.chrome.com/docs/extensions/reference/api/declarativeNetRequest#safe_rules) can be updated, limiting the update frequency of lists using advanced techniques.
