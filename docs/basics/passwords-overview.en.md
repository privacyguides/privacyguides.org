...
Each word in the aforementioned list results in **~12.9** bits of entropy. A seven word passphrase derived from it has **~90.47** bits of entropy. Let's put all of this in perspective: the list that we used contains 7776 unique words. A seven word passphrase using that list is one of **~1,719,070,799,748,422,500,000,000,000** possible passphrases. That is calculated by doing $WordsInList^{WordsInPhrase}$, or in our case $7776^7$. On average, it takes trying 50% of all the possible combinations to guess your phrase. With that in mind, even if your adversary is capable of **~1,000,000,000,000** guesses per second, it would still take them **~27,255,689 years** to guess your passphrase. That is the case even if the following things are true:
