...
## Aperçu du Chiffrement des Emails

La méthode standard pour ajouter du E2EE aux emails entre différents fournisseurs mails est d'utiliser OpenPGP. Il existe différentes implémentations de la norme OpenPGP, les plus courantes étant [GnuPG](https://fr.wikipedia.org/wiki/GNU_Privacy_Guard) et [OpenPGP.js](https://openpgpjs.org).

Il existe une autre norme populaire auprès des entreprises, appelée [S/MIME](https://fr.wikipedia.org/wiki/S/MIME), mais elle nécessite un certificat émis par une [Autorité de Certification](https://fr.wikipedia.org/wiki/Autorit%C3%A9_de_certification) (toutes ne délivrent pas de certificats S/MIME). Elle est prise en charge par [Google Workplace](https://support.google.com/a/topic/9061730?hl=fr&amp%3Bref_topic=9061731) et [Outlook sur le Web ou Exchange Server 2016, 2019](https://support.microsoft.com/fr-fr/office/chiffrer-les-messages-%C3%A0-l-aide-de-s-mime-dans-outlook-sur-le-web-878c79fc-7088-4b39-966f-14512658f480?ui=en-us&rs=en-us&ad=us).

Même si vous utilisez OpenPGP, il ne prend pas en charge la [confidentialité persistante](https://fr.wikipedia.org/wiki/Confidentialit%C3%A9_persistante), ce qui signifie que si votre clé privée ou celle du destinataire est volée, tous les messages précédents chiffrés avec cette clé seront exposés. C'est pourquoi nous recommandons, dans la mesure du possible, les [messageries instantanées](../real-time-communication.md) qui mettent en œuvre la confidentialité persistante par rapport aux emails pour les communications de personne à personne.
...
Les métadonnées des emails sont stockées dans [l'en-tête de message](https://fr.wikipedia.org/wiki/Courrier_%C3%A9lectronique#Exemple_de_contenu_brut_d'un_courrier_%C3%A9lectronique) de l'email et comprennent certains en-têtes visibles que vous avez peut-être vus, tels que : `À`, `De`, `Cc`, `Date`, `Sujet`. Il existe également un certain nombre d'en-têtes cachés inclus par de nombreux clients et fournisseurs de messagerie qui peuvent révéler des informations sur votre compte.
