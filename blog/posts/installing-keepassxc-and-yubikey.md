...
If you are looking for a good remote password manager you can use from anywhere, there are plenty of excellent options to [choose](https://www.privacyguides.org/en/passwords/) from. But if you prefer to keep your passwords stored locally only, [KeePassXC](https://keepassxc.org/) is what you need. In this tutorial, we will set up KeePassXC to work with [YubiKey](https://www.yubico.com/) as an additional factor to secure your local-only password database.<!-- more -->

## :simple-keepassxc: KeePassXC summary

KeePassXC is a free, open-source, and desktop-only password manager. The community-driven project was first released in 2012 and is a port of the application [KeePass Password Safe](https://en.wikipedia.org/wiki/KeePass) and [KeePassX](https://en.wikipedia.org/wiki/KeePassX), which is no longer actively maintained.
...
On March 4th, 2025, KeePassXC released its most recent update. This update includes the capacity to import Proton Pass databases, to generate passphrase using *mixed* case (a mix of uppercase and lowercase), and a lot of other [useful features](https://keepassxc.org/blog/2025-03-04-2.7.10-released/).
...
This tutorial was done from macOS, but your experience shouldn't be much different if you are using Linux or Windows.
...
Thankfully for this tutorial, there is a way to disabled it temporarily, but **you** should definitely keep it on.
...
A database in KeePassXC is an encrypted file that will contain all the passwords you decide to register there.

You can use multiple separated databases with KeePassXC. For example, you could have a database for work, a database for your family, and a database for your personal accounts. All stored in separated files with separated main passwords. In the application, each database can be opened on its own tab.
...
This step isn't completed, but this is where you will be adding your YubiKey to secure further your database. Keep the "Database Credentials" application window open and **continue with the step below** :material-arrow-down-bold:
...
The Challenge-Response will remain the same each time you decrypt your database, *but*, it will change each time the database is updated (each time there is a change to it, such as adding an entry, removing an entry, adding a note, etc.). Note that the previous versions of your database could get unlocked with your main password + your key's previous Challenge-Response, however.
...
If you do have two YubiKeys and want to learn how to **reset your YubiKey entirely and create a backup of it** on a second (or more) keys, you can [read our Privacy Guides tutorial](https://www.privacyguides.org/articles/2025/03/06/yubikey-reset-and-backup/) on this.

If you only need to learn more about the Challenge-Response YubiKey application, jump to [this section](https://www.privacyguides.org/articles/2025/03/06/yubikey-reset-and-backup/#step-9-create-and-clone-your-keys-challenge-response) of the tutorial directly.

### Step 5: Add your YubiKey

Once you YubiKey's Challenge-Response slot has been properly configured and backed up, return to the KeePassXC's "Database Credentials" window, and click on the "Add additional protection" button in the middle.
...
You should see your YubiKey's model and serial number listed, and also in which YubiKey slot you have stored your Challenge-Response. Once the correct key is selected, click on "Done" at the bottom.
...
This means that for any Firefox-based browser (including Tor), you can click on the Firefox logo, and for any Chromium-based browser, you can click on the Chrome logo. Some browsers might not be supported, however.
...
Additionally, the more unique your combination of hardware, software, and browser extensions is, the more you are vulnerable to [browser fingerprinting](https://apertatube.net/w/fdszTYBKzeoE3ySQUGTzmo). Always be mindful to consider your specific threat model when installing new browser extensions.
...
Make sure your KeePassXC database is *unlocked*, then **restart you browser**.
